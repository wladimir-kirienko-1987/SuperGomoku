...
void show_cross(int i, int j)
{
	...
	apply_surface( ( ( i * (board_->w / Board::FIELD_SIZE) ) + ( SCREEN_WIDTH - board_->w ) / 2 ) + 2, 
		( ( j * (board_->h / Board::FIELD_SIZE) ) + ( SCREEN_HEIGHT - board_->h ) / 2 ) + 2, 
		cross, screen );
}
